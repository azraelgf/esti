(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            const activeMenu = document.querySelector("._submenu-active"), activeMenuSub = document.querySelector("._submenu-open"), activeMenuInSub = document.querySelector("._insubmenu-open");
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                bodyLockToggle();
                document.documentElement.classList.toggle("menu-open");
                activeMenu ? activeMenu.classList.remove("_submenu-active") : null;
                activeMenuSub ? activeMenuSub.classList.remove("_submenu-open") : null;
                activeMenuInSub ? activeMenuInSub.classList.remove("_insubmenu-open") : null;
            }
        }));
    }
    function menuClose() {
    ...
    class SelectConstructor {
        ...
        setupPlaceholder(selectItem, originalSelect) {
            ...
            if (placeholder) {
                ...
                if (placeholder.label.show) {
                    ...
                    selectItemTitle.insertAdjacentHTML("afterbegin", `<span class="${this.selectClasses.selectLabel}">${placeholder.label.text || placeholder.value}</span>`);
                }
            }
        }
        buildSelectStructure(selectItem) {
            selectItem.insertAdjacentHTML("beforeend", `<div class="${this.selectClasses.selectBody}"><div hidden class="${this.selectClasses.selectOptions}"></div></div>`);
        }
        ...
        selectsActions(e) {
            ...
            if (selectItem) {
                ...
                if (targetType === SELECT_EVENTS.CLICK && !originalSelect.disabled) this.handleClick(targetElement, selectItem, originalSelect); else if (targetType === SELECT_EVENTS.FOCUSIN || targetType === SELECT_EVENTS.FOCUSOUT) this.handleFocus(selectItem, targetType); else if (targetType === SELECT_EVENTS.KEYDOWN && e.code === "Escape") this.selectsClose();
            } else this.selectsClose();
        }
        getClosestSelect(targetElement) {
            return targetElement.closest(this.getSelectClass(this.selectClasses.select)) || targetElement.closest(this.getSelectClass(this.selectClasses.selectTag)) && document.querySelector(`.${this.selectClasses.select}[data-id="${targetElement.closest(this.getSelectClass(this.selectClasses.selectTag)).dataset.selectId}"]`);
        }
        handleClick(targetElement, selectItem, originalSelect) {
            ...
            } else if (targetElement.closest(this.getSelectClass(this.selectClasses.selectTitle))) this.selectAction(selectItem); else if (targetElement.closest(this.getSelectClass(this.selectClasses.selectOption))) {
        ...
        handleFocus(selectItem, targetType) {
            if (targetType === SELECT_EVENTS.FOCUSIN) selectItem.classList.add(this.selectClasses.selectFocus); else selectItem.classList.remove(this.selectClasses.selectFocus);
        }
        ...
        selectAction(selectItem) {
            ...
            if (originalSelect.closest("[data-one-select]")) this.selectsClose(originalSelect.closest("[data-one-select]"));
            ...
            if (isMobileDevice && originalSelect.dataset.mobilePopup) this.showMobilePopup(selectItem, selectOptions); else this.toggleDesktopSelect(selectItem, selectOptions, selectOpenzIndex);
        }
        ...
        getSelectTitleValue(selectItem, originalSelect) {
            ...
            if (originalSelect.multiple && originalSelect.hasAttribute("data-tags")) {
                selectTitleValue = this.getSelectedOptionsData(originalSelect).elements.map((option => `<span role="button" data-select-id="${selectItem.dataset.id}" data-value="${option.value}" class="${this.selectClasses.selectTag}">${this.getSelectElementContent(option)}</span>`)).join("");
            ...
            if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.selectTitle}"><span${pseudoAttribute} class="${this.selectClasses.selectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.selectInput}"></span></div>`;
        ...
        getSelectedOptionsData(originalSelect, type) {
            let selectedOptions = originalSelect.multiple ? Array.from(originalSelect.options).filter((option => option.value && option.selected)) : [ originalSelect.options[originalSelect.selectedIndex] ];
            return {
        ...
        getOption(selectOption, originalSelect) {
            ...
            let selectOptionHTML = selectOptionLink ? `<a ${selectOptionLinkTarget} ${selectOptionHide} href="${selectOptionLink}" data-value="${selectOption.value}" class="${this.selectClasses.selectOption}${selectOptionClass}${selectOptionSelected}">` : `<button ${selectOptionHide} class="${this.selectClasses.selectOption}${selectOptionClass}${selectOptionSelected}" data-value="${selectOption.value}" type="button">`;
        ...
        log(message, type = "log") {
            if (this.config.logging) console[type](`[SelectConstructor] ${message}`);
        }
    }
    ...
    function initSliders() {
        ...
        if (document.querySelector(".equipment__body")) new Swiper(".equipment__body", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".licenses__body")) new Swiper(".licenses__body", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".before-after__body")) new Swiper(".before-after__body", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
...
***************************************************************************** */
        ...
        galleries.forEach((gallery => {
            galleyItems.push({
                ...
                galleryClass: lightgallery_es5(gallery, {
                    selector: ".licenses-item__img img",
    ...
    da.init();
    if (window.matchMedia("(min-width: 991.98px)").matches) document.addEventListener("mouseover", documentHovers);
    document.addEventListener("click", documentActions);
    function documentHovers(e) {
        const targetElement = e.target;
        if (targetElement.closest("[data-parent]")) {
            const activeInLink = document.querySelector("._insubmenu-open--active"), activeInBlock = document.querySelector("._insubmenu-open");
        ...
        if (targetElement.closest("[data-inparent]")) {
            const numMenu = targetElement.dataset.inparent ? targetElement.dataset.inparent : null, subMenu = document.querySelector(`[data-insubmenu="${numMenu}"]`);
            if (subMenu) {
                const activeLink = document.querySelector("._insubmenu-open--active"), activeBlock = document.querySelector("._insubmenu-open");
                if (activeLink && activeLink !== targetElement) {
    ...
    function documentActions(e) {
        ...
        if (window.matchMedia("(max-width: 991.98px)").matches) {
            if (targetElement.closest(".has-menu")) {
                const firstSubMenu = document.querySelector(".submenu-wrapper__item:first-child"), wrapperSub = document.querySelector(".submenu-wrapper");
